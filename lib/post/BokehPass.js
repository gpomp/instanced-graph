...
function BokehPass ( scene, camera, params = {} ) {
  ...
  this.vectorZero = new THREE.Vector3(0, 0, 0);
...
BokehPass.prototype = {
  ...
  _updateTargets: function (renderTarget) {
    ...
    if (!this.renderTargetColor) {      
      ...
      this.renderTargetDepth.setSize(downWidth, downHeight);
    }
  },

  render: function (renderer, writeBuffer, readBuffer, delta, maskActive, depthTexture) {
    var distance = clamp(this.camera.position.distanceTo(this.vectorZero), 1.0, 500.0) / 500;
    this.uniforms.focus.value = distance * 0.5;
    this.uniforms.aperture.value = (1 - distance) * 0.05;
