...
function BloomPass (scene, camera, opt = {}) {
  ...
  this.materialBlur.name = 'bloom-blur-material';
  ...
  this.blurStrength = 1.5;

  this.postCamera = new THREE.OrthographicCamera(-1, 1, 1, -1, 0, 1);
  this.postScene = new THREE.Scene();

  this.postQuad = new THREE.Mesh(new THREE.PlaneBufferGeometry(2, 2), this.combineShader);
...
BloomPass.prototype = {
  ...
  _updateTargets: function (renderTarget) {
    ...
    if (!this._thresholdTarget || !this._blurTarget) {      
      ...
      this.materialBlur.uniforms.resolution.value.set(downWidth, downHeight);
  ...
  render: function (renderer, writeBuffer, readBuffer, delta) {
    ...
    this.postQuad.material = this.materialBlur;
    ...
    renderer.render( this.postScene, this.postCamera, this._blurTarget, false );

    this.materialBlur.uniforms.tDiffuse.value = this._blurTarget.texture;
    this.materialBlur.uniforms.direction.value.set(0.0, this.blurStrength);
    renderer.render( this.postScene, this.postCamera, this._blurTarget2, false );

    this.combineShader.uniforms.tDiffuse.value = readBuffer.texture;
    this.combineShader.uniforms.tBloomDiffuse.value = this._blurTarget2.texture;

    var dpr = renderer.getPixelRatio();
    this.postQuad.material = this.combineShader;
